...
(async function() {
  ...
  await Promise.all(files.map(async file => {
    const BASE_PATH = dirname(file);
    const OUTPUT_FILE = join('dist', BASE_PATH, `${BASE_PATH.split(sep)[1]}.js`)
    ...
    await copyFile(join(BASE_PATH, 'index.html'), join(dirname(OUTPUT_FILE), 'index.html'))
  }))
})()